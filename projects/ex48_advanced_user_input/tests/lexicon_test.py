import pytest
from ex48_advanced_user_input import lexicon

def test_directions():
    # scan function is called on a word. Returns a list of one tuple.
    assert lexicon.scan("north") == [('direction', 'north')]

    # scan function is called on a sentence. Returns a list of tuples.
...
def test_errors():
    ...
    result = lexicon.scan("bear IAS princess 1235 bear")
    assert result == [('noun', 'bear'),
                      ('error', 'IAS'),
                      ('noun', 'princess'),
                      ('number', 1235),
